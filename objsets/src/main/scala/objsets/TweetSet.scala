...
abstract class TweetSet {
  def RetweetAcc(value: Tweet):Tweet= value
  ...
  def descendingHelper(twSet: TweetSet, acc: TweetList): TweetList

//  def ascendinHelper(twSet: TweetSet, acc: TweetList): TweetList={
//    if(twSet.isEmpty) acc else ascendinHelper(remove(leastRetweeted),new Cons(leastRetweeted,acc.tail))
//  }
...
class Empty extends TweetSet {
  ...
  def descendingHelper(twSet: TweetSet, acc: TweetList): TweetList= acc
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingHelper(twSet: TweetSet, acc: TweetList): TweetList= {
    if (twSet.isEmpty) acc
    else {
      val most = twSet.mostRetweeted
      descendingHelper(twSet.remove(most), new Cons(most, acc))
    }
  }
  ...
  override def RetweetAcc(max: Tweet): Tweet ={
    left.RetweetAcc(right.RetweetAcc(if (elem.retweets > max.retweets) elem else max))
  }

//  def leastRetweeted = RetweetAcc(this,elem,tweet => tweet.retweets < elem.retweets)
}
